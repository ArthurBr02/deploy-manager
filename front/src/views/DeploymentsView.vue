  ...
  <div class="flex flex-col h-full">
    <header class="h-14 border-b border-warm-border bg-white flex items-center px-4 lg:px-6 gap-4 flex-shrink-0">
      ...
      <button @click="exportCsv" class="flex items-center gap-1.5 px-3 py-1.5 border border-warm-border rounded-md text-sm bg-white hover:bg-warm-muted">
        <DownloadIcon class="w-3.5 h-3.5" /> <span class="hidden sm:inline">Exporter CSV</span>
      </button>
    </header>
    ...
    <div class="flex-1 overflow-auto p-4 lg:p-6 space-y-4">
      ...
      <div class="bg-white border border-warm-border rounded-xl px-4 py-3 flex flex-wrap gap-2.5 items-center">
        ...
        <select v-model="filters.hostId" class="flex-1 sm:flex-none border border-warm-border rounded-md px-2 py-1.5 text-sm outline-none focus:border-accent">
          <option value="">Tous les hôtes</option>
          <option v-for="h in hosts" :key="h.id" :value="h.id">{{ h.name }}</option>
        </select>
        <select v-model="filters.status" class="flex-1 sm:flex-none border border-warm-border rounded-md px-2 py-1.5 text-sm outline-none focus:border-accent">
          <option value="">Tous les statuts</option>
          <option v-for="s in statuses" :key="s.value" :value="s.value">{{ s.label }}</option>
        </select>
        <select v-model="filters.type" class="flex-1 sm:flex-none border border-warm-border rounded-md px-2 py-1.5 text-sm outline-none focus:border-accent">
          <option value="">Tous les types</option>
          <option value="DEPLOY">Déployer</option>
          <option value="GENERATE">Générer</option>
          <option value="DELIVER">Livrer</option>
        </select>
        <select v-model="filters.period" class="flex-1 sm:flex-none border border-warm-border rounded-md px-2 py-1.5 text-sm outline-none focus:border-accent">
      ...
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div v-for="s in statsCards" :key="s.label" class="bg-white border border-warm-border rounded-xl p-4">
          <div class="text-[10px] text-gray-400 uppercase tracking-wide truncate">{{ s.label }}</div>
          <div class="text-xl sm:text-2xl font-semibold mt-1 tracking-tight" :style="s.color ? `color: ${s.color}` : ''">
            {{ stats ? s.value(stats) : '—' }}
          </div>
          <div class="text-[10px] sm:text-xs text-gray-400 mt-0.5 truncate">{{ stats ? s.sub(stats) : '' }}</div>
        </div>
      </div>
  ...
  computed: {
    ...
    statsCards() {
      return [
        ...
        { label: 'Réussis', value: s => s.success, sub: s => `${s.total > 0 ? Math.round(s.success / s.total * 100) : 0} %`, color: 'var(--color-status-success, #16a34a)' },
        ...
        { label: 'Médiane', value: s => s.medianDuration ?? '—', sub: () => '' },
      ]
    },
  },
  watch: {
